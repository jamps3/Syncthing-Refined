...
- Preparation
  ...
  - Syncthing Refined will require the `config.zip` to restore your config.

- Migration
  - Install our [latest release](https://github.com/researchxxl/syncthing-android/releases/latest) from GitHub or F-Droid.
  - Start Syncthing Refined.
  - Complete the welcome slides.
  - In the Syncthing Refined settings, go to "Import & Export".
    - Import the `config.zip` you created earlier.
      - 🍀 Don't worry. You will get an error message, but the import worked. That's because some Syncthing Refined specific stuff wasn't found during the import and doesn't hurt.
...
- Further advice
  - 🐞 You won't be able to open Syncthing's web UI. Instead, you are presented a password prompt.
    - In the Syncthing Refined settings, go to "Syncthing options".
